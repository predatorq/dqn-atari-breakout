...
class Agent(object):
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        y_batch = []
        current = self.__policy(next_batch).cpu().data.numpy()
        max_a = numpy.argmax(current, axis=1)
        targetq = self.__target(next_batch)
        
        for i in range(batch_size):
            if done_batch[i]:
                y_batch.append(reward_batch[i])
            else:
                targetq_v = targetq[i, max_a[i]]
                y_batch.append(reward_batch[i] + self.__gamma * targetq_v)
